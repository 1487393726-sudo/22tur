...
export async function GET(
  ...
  try {
    ...
    // 生成发票内容
    const invoiceContent = generateInvoice(purchase);

    // 返回HTML格式的发票
    return new NextResponse(invoiceContent, {
      headers: {
        'Content-Type': 'text/html; charset=utf-8',
        'Content-Disposition': `attachment; filename="purchase-invoice-${purchase.id}.html"`,
      },
    });
  } catch (error) {
 ...
 * 生成发票HTML内容
 */
function generateInvoice(purchase: any): string {
  ...
  const invoiceNumber = `INV-${purchase.id.substring(0, 8).toUpperCase()}-${new Date(purchase.purchaseDate).getFullYear()}`;
  const userName = `${purchase.user.firstName}${purchase.user.lastName}`;
  const tax = purchase.amount * 0.06; // 6% 税率
                ...
                <table>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>标准版</td>
                            <td>项</td>
                            <td class="amount-right">¥${purchase.amount.toLocaleString('zh-CN')}</td>
            ...
            <div class="summary">
                <div class="summary-table">
                    <div class="summary-row">
                        <span>小计：</span>
                        <span>¥${purchase.amount.toLocaleString('zh-CN')}</span>
                    </div>
                    <div class="summary-row">
                        <span>税率（6%）：</span>
